...
export default async function handler(req, res) {
  ...
  email.subject  = 'Your Clausule sign-in code'
  email.sender   = { name: 'Clausule', email: 'noreply@clausule.app' }
  email.to       = [{ email: to }]
  email.htmlContent = `
  ...
  try {
    ...
    console.error('[send-code] Brevo error:', err?.response?.body ?? err.message)
