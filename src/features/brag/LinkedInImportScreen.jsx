...
export default function LinkedInImportScreen({ initialSession = null, initialError = '' }) {
  ...
  const connectMutation = useMutation({
    mutationFn: () => apiJson('/api/brag/linkedin/imports', jsonRequest({}, { method: 'POST' })),
  })

  const publishMutation = useMutation({
    mutationFn: (sessionId) => apiJson(`/api/brag/linkedin/imports/${sessionId}/publish`, jsonRequest({}, { method: 'POST' })),
  })
  ...
  const handleToggle = async (item) => {
    ...
    try {
      const result = await apiJson(
        `/api/brag/linkedin/imports/${session.id}`,
        jsonRequest({ itemId: item.id, selected: nextSelected }, { method: 'PATCH' })
      )
      syncSession(result.session)
    } catch {
  ...
  const handleSkip = async () => {
    if (session?.id) {
      try {
        await apiJson(`/api/brag/linkedin/imports/${session.id}`, jsonRequest({ status: 'skipped' }, { method: 'PATCH' }))
      } catch {
