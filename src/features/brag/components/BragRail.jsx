...
export default function BragRail({ activePage }) {
  ...
  return (
    <aside className={bragShell.rail} aria-label="App navigation">
      <img className={bragShell.railLogo} src="/favicon.svg" alt="" aria-hidden="true" />
      <nav className={bragShell.railNav} aria-label="Primary">
        {!mfaSetupRequired && (
          navItems.map(({ page, href, label, icon }) => (
            <Link
              ...
              className={cn(
                bragShell.railBtn,
                activePage === page && bragShell.railBtnActive
              )}
              aria-label={label}
      ...
      <div className={bragShell.railFoot}>
        <button type="button" onClick={logout} className={bragShell.railIconBtn} aria-label="Sign out">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
