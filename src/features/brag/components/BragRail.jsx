...
export default function BragRail({ activePage }) {
  ...
  return (
    <aside
      className={cn(
        'sticky top-0 flex h-screen w-[46px] shrink-0 flex-col items-center bg-[var(--be-rail-surface)] px-0 py-[18px] text-[var(--be-rail-icon)]',
    ...
    >
      <img className="mb-[20px] block h-7 w-7 select-none rounded-[7px] max-sm:hidden" src="/favicon.svg" alt="" aria-hidden="true" />
        ...
        {!mfaSetupRequired && (
          navItems.map(({ page, href, label, icon }) => (
            <Link
              ...
              href={href}
              className={cn(
                'flex h-[34px] w-[34px] items-center justify-center rounded-[var(--r)] border-none bg-transparent no-underline transition-colors duration-150 max-sm:h-11 max-sm:w-11',
                activePage === page
                  ? 'bg-[var(--be-rail-active-bg)] text-[var(--be-rail-icon-hover)]'
                  : 'text-[var(--be-rail-icon)] hover:bg-white/5 hover:text-[var(--be-rail-icon-hover)]'
              )}
              aria-label={label}
      ...
      <div className="mt-2 flex flex-col items-center gap-2 max-sm:hidden">
        <button type="button" onClick={logout} className="flex h-7 w-7 items-center justify-center border-none bg-transparent text-[var(--be-rail-icon)] transition-colors duration-150 hover:text-[var(--be-rail-icon-hover)]" aria-label="Sign out">
