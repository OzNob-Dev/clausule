...
export default function BragIdentitySidebar({
  ...
  return (
    <aside
      className="sidebar sticky top-0 flex h-screen w-80 shrink-0 flex-col overflow-y-auto border-r-[1.5px] border-r-[var(--cl-black-20)] bg-[var(--sidebar-bg)] text-[var(--sidebar-text)] max-[768px]:static max-[768px]:h-auto max-[768px]:w-full"
      ...
      <nav className="sidebar__nav flex flex-1 flex-col py-6" aria-label="Primary">
        {NAV_SECTIONS.map((section) => (
          <section key={section.title} className="sidebar__section mb-8 flex flex-col gap-3 last:mb-0" aria-labelledby={`sidebar-${section.title.toLowerCase().replace(/\s+/g, '-')}-title`}>
            <div id={`sidebar-${section.title.toLowerCase().replace(/\s+/g, '-')}-title`} className="sidebar__section-title px-7 text-[var(--cl-text-sm)] font-bold uppercase tracking-[0.08em] text-[var(--sidebar-accent)] max-[768px]:px-5">{section.title}</div>
              ...
              {section.items.map((item) => (
                <li key={item.page} className="sidebar__item">
                  <Link
                    href={item.href}
                    className={cn(
                      'sidebar__link flex items-center gap-3 border-l-[3px] border-l-transparent px-7 py-3 text-[var(--cl-text-lg)] font-medium text-[var(--sidebar-text)] no-underline transition-[color,background-color,border-color] duration-200 hover:border-l-[var(--sidebar-accent)] hover:bg-[var(--sidebar-bg-hover)] hover:text-[var(--sidebar-text-strong)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[var(--sidebar-focus)] max-[768px]:px-5 motion-reduce:transition-none',
                      (activePage === item.page || item.children?.some((child) => activeChildPage === child.page)) && 'sidebar__link--active border-l-[var(--sidebar-accent)] bg-[var(--sidebar-accent-soft)] text-[var(--sidebar-text-strong)] font-bold'
                    )}
                    ...
                    <ul className="sidebar__sublist mt-1 m-0 list-none p-0" aria-label={`${item.label} child pages`}>
                      {item.children.map((child) => (
                        <li key={child.page} className="sidebar__subitem">
                          <Link
                            href={child.href}
                            className={cn('sidebar__sublink block px-7 py-2 pl-[60px] text-[var(--cl-text-base)] text-[var(--sidebar-text-muted)] no-underline transition-[color,background-color] duration-200 hover:bg-[var(--cl-white-4)] hover:text-[var(--sidebar-text-strong)] max-[768px]:px-5 max-[768px]:pl-[52px] motion-reduce:transition-none', activeChildPage === child.page && 'sidebar__sublink--active text-[var(--sidebar-accent)] font-bold')}
